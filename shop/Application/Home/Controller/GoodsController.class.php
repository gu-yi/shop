...
class GoodsController extends Controller
{
    ...
    $data = D('goodsView')->where(array('id='.$id,'goods_status=1'))->find();
      ...
      foreach ($goods_attr as $key=>$value){
        ...
        $datas = D('GoodsOrder')->alias('a')->field('a.*,b.user_name,c.order_addtime')->join(' left join tp_user as b on a.user_id = b.id, tp_order as c ')->where(array('a.order_id'=>'c.id','a.og_goods_id'=>$id))->select();
        //dump($datas);die;
